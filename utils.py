...
import random
# 使用相對匯入避免在未安裝套件時產生錯誤
from .config import SPECIAL_RENDER_OVERRIDES, CHAR_SPACING
...
def get_spacing(ch):
    ...
    if ch == ' ':
        base = 500
    elif ch in SPECIAL_RENDER_OVERRIDES:  # 特殊符號
        base = SPECIAL_RENDER_OVERRIDES[ch].get("spacing", -5)
    elif '\u4e00' <= ch <= '\u9fff':  # 中文
        base = 5
    elif ch.isdigit() or ch.isalpha():  # 英文或數字
        base = -60
    else:
        base = 10
    return base + CHAR_SPACING
